...
MONITORING_URL = os.getenv("MONITORING_URL", "http://localhost:8080")
TIMEOUT_SECONDS = 8
...
st.write("")
ops_tab, monitoring_tab, details_tab = st.tabs(["Advisor workspace", "Monitoring", "Technical details"])
...
with monitoring_tab:
    st.markdown(
        '<div class="section-note">Model and data quality reports generated by Evidently. Run <code>make run-monitoring</code> first to generate the reports.</div>',
        unsafe_allow_html=True,
    )
    st.write("")
    m1, m2 = st.columns(2)
    with m1:
        st.markdown(
            """
            <div class="mini-card">
                <div class="summary-label">Data drift</div>
                <div class="summary-value" style="font-size:1.35rem;">Feature distribution</div>
                <div class="summary-meta" style="margin-bottom:0.9rem;">Compares training vs validation distributions across all features. Flags statistical drift for each input.</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        st.link_button(
            "Open data drift report",
            f"{MONITORING_URL}/data_drift.html",
    ...
    with m2:
        st.markdown(
            """
            <div class="mini-card">
                <div class="summary-label">Model performance</div>
                <div class="summary-value" style="font-size:1.35rem;">Classification metrics</div>
                <div class="summary-meta" style="margin-bottom:0.9rem;">Accuracy, precision, recall, F1 and confusion matrix on training vs validation. Detects model degradation.</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        st.link_button(
            ...
            f"{MONITORING_URL}/classification_performance.html",
            use_container_width=True,
        )
